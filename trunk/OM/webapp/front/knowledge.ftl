			...
			<#list knowledgeMap.knowLedgeList.list as list>
       			<tbody>
          			<tr class="light">
            			...
            			<#if loginUser?exists>
            				<td><a href="${base}/front/Member!viewKnowledgeDetail.action?id=${list.Id}" target="_black">查看</a></td>	
          				<#else>
      ...
      <div class="subnav">
        ...
        <#if knowledgeMap.knowLedgeDivision?has_content>
			<#list knowledgeMap.knowLedgeDivision as list>
          		<#if loginUser?exists>
					<li><a href="${base}/front/Member!viewKnowledgeDivision.action?id=${list.id}">${list.name?default('-')}</a></li>
				<#else>

...
function validateForm(theForm) {
	...
	for(var i = 0; i < field.length; i++) {
		...
		if (dateInfo!=null) {
			if (value!="") {
				var reg = new RegExp("^[0-9]{4}-((0[1-9])|1[0-2])-((0[1-9])|([1-2][0-9])|30|31)$");
				if (reg.test(value)==false) {
					alert(dateInfo);
					field[i].select();
					return false;
				...
				var m = parseInt(value.substr(5,2),10);
				var d = parseInt(value.substr(8,2),10);
...
/* 联动菜单 */
function loadOption(firstSelId, secondSelId, key, url) {
	var mainVal = $("#" + firstSelId).val();
    $("#" + secondSelId)[0].options.length = 1;
	$("#" + secondSelId).change();

	if (mainVal != "") {
		$.getJSON(url + "?" + key + "=" + mainVal + "&timestampnocache=" + new Date().getTime(),
            function(data){
				$("#" + secondSelId).addOption(data, false);
			}
		);
	}
}
